...
/**
 * Renders a complete testimonial card onto a canvas to be used as a WebGL texture.
 */
function createCardCanvas(data, width = 800, height = 600) {
    ...
    ctx.fillText(data.name || 'Anonymous', width / 2, 140);

    // 4. Role (Medium, Subtle)
    ctx.font = '500 18px "Poppins", sans-serif';
    ctx.fillStyle = '#94a3b8';
    ctx.fillText(data.role || 'Client', width / 2, 190);

    // 5. Testimonial Text (Wrapped)
    ...
    wrapText(ctx, text, x, yStart, maxWidth, lineHeight, 4);
...
class Media {
    ...
    createShader() {
      ...
      `,
        ...
        const canvas = createCardCanvas(this.data);
...
class App {
    ...
    onWheel(e) {
        // Optional: you can keep wheel interaction or disable it.
        // I will keep it but it moves the target normally.
    ...
    update() {
        // Increment target for auto-scroll if not paused and not being dragged
    ...
    addEventListeners() {
        ...
        // Use the container for the click to avoid global page click issues
